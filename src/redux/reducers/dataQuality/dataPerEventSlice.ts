  ...
  async (_temp, { rejectWithValue, getState }) => {
    ...
    try {
      ...
      data.EventParameters =
        eventParamsData.overall_events_percentage ??
        dataPerEventsInitialState.EventParameters;
      data.attribution =
        eventAttributionData.total_overall_attribution_percentage ??
        dataPerEventsInitialState.attribution;
      data.event =
        eventParamsData.total_overall_events_percentage ??
        dataPerEventsInitialState.event;
      data.byDate = (
        eventAttributionData.grouped_attribution_percentage ??
        dataPerEventsInitialState.byDate
      )
        .map(
          (
            {
            ...
            if (
              ...
              eventParamsData.grouped_events_percentage[idx].events_quality ===
                0
            )
              return null;
            return {
