...
>('user/login', async ({ email, password, rememberMe }, { rejectWithValue }) => {
  try {
    const res = await axios.post(`${process.env.NEXT_APP_BFF_URL}/auth/login`, {
      email,
      password
    });
...
>('user/register', async (formData, { rejectWithValue }) => {
  try {
    const res = await axios.post(`${process.env.NEXT_APP_BFF_URL}/auth/register`, formData);
...
>('user/authenticate', async (token, { rejectWithValue }) => {
  try {
    if (token) {
      const res = await axios.get(`${process.env.NEXT_APP_BFF_URL}/auth/me`, {
        headers: {
          Authorization: token
        }
      });
