...
const ParameterStat = ({ name, value }: ParameterStatProps) => (
  <div className="py-2  flex items-center justify-between bg-gradient-to-r from-[#1C2B43] to-bgContainerFrom rounded-[.3125rem] border-[.0625rem] border-light/10 text-light px-5   gap-[.3125rem]">
    <div className="text-[.8rem] 2xl:text-[.90rem] hd:text-[1.2rem] w-30">{name}</div>
    <div className="flex  text-[1rem]  h-full items-center justify-between flex-grow w-70 gap-2">
      ...
      <div className="w-20 px-1">
        <span className="font-semibold text-[.8rem] 2xl:text-[.90rem] hd:text-[1.3rem] ">
          {Math.floor((value ?? 0) * 100)}
        </span>
        <span className="text-[1rem] hd:text-[1.4rem]">%</span>
      </div>
    </div>
  </div>
);
