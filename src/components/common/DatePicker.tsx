...
const DatePicker = ({ close, isOpen }: datePickerProps) => {
  ...
  const [selectedDateRange, setSelectedDateRange] =
    useState<(Moment | undefined)[]>(dateRange);
  ...
  return (
    <ComponentWrapper
      width={1000}
      className="!rounded-[10px] shadow-xl shadow-background"
    >
      <div className="flex flex-row justify-between h-full gap-4">
          ...
          <DateRange
            onChange={(ranges) => {
              ...
              if (selection.startDate)
                selected.push(moment(selection.startDate));
        ...
        <div className="flex flex-col pl-[50px] pr-[120px] border-l-[1px] border-lines/[0.2] gap-[20px] text-[16px] leading-[19px]">
          <h5 className="font-lato font-bold text-primary mb-[4px] ">
            Presets
          </h5>
